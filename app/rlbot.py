...
class CustomEnv:
    ...
    def reset(self, env_steps_size=0):
        ...
        for i in reversed(range(self.lookback_window_size)):
            ...
            self.market_history.append(
                [self.df_normalized.loc[current_step, column] for column in self.columns])

        state = np.concatenate(
            (self.orders_history, self.market_history), axis=1)
    ...
    def next_observation(self):
        self.market_history.append(
            [self.df_normalized.loc[self.current_step, column] for column in self.columns])
        obs = np.concatenate(
            (self.orders_history, self.market_history), axis=1)
...
def test_agent(test_df, test_df_nomalized, visualize=True, test_episodes=10, folder="", name="", comment="", Show_reward=False, Show_indicators=False):
    ...
    env = CustomEnv(df=test_df, df_normalized=test_df_nomalized,
                    lookback_window_size=params["lookback window size"], Show_reward=Show_reward, Show_indicators=Show_indicators)
...
if __name__ == "__main__":
    ...
    ##### TESTING #####
    training_folder = "2021_06_13_23_06_Crypto_trader"
    ...
    for root, directories, files in os.walk("runs/"+training_folder+"/models", topdown=True):
        for file in files:
            if(file[-10:] != "_Critic.h5"):
                ...
                test_multiprocessing(CustomEnv, CustomAgent, train_df, train_df_nomalized, num_worker=16, visualize=False,
                                     test_episodes=500, folder="runs/"+training_folder, name=model, comment="Bull Market 1")
